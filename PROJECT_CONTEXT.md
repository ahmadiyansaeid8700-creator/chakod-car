...
- مسیرهای اصلی بازار و جزئیات خودرو اکنون `/cars`، `/cars/luxury`، `/cars/free-zone` و `/cars/[slug]` هستند.
...
- ۹ سناریوی HTTP برای مسیرهای اصلی و قدیمی موفق و ۱۹ تست مستقل پروژه بعد از این Patch موفق شدند.
...
- مسیر نهایی خودروها تعیین و تثبیت شده است.
- گام بعدی، یکسان‌سازی ثبت و مدیریت آگهی‌ها روی `/account/listings/*` است.
