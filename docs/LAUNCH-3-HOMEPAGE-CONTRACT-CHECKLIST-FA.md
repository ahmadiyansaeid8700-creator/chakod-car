...
- [x] Resolve پیش‌فرض ویندوز پس از تغییر DNS دوباره اجرا شد و `api.chakod.com` به `172.67.204.1` و `104.21.77.33` Resolve شد.
- [!] داده واقعی خودروهای لوکس هنوز تایید نشده است؛ تست اتصال و پاسخ API پس از اصلاح DNS باقی مانده است.
- [!] پاسخ قبلی `10.10.34.35` از DNS مودم یا Resolver بالادستی آن باعث Timeout مسیر API شده بود.
- [!] Console در Runtime پاسخ‌های `502` و `522` برای API کسب‌وکارها، استوری‌ها، ذخیره آگهی و API عمومی چاکود نشان داده بود؛ پس از اصلاح DNS باید دوباره بررسی شود.
- [~] اتصال TCP پورت `443` پس از اصلاح DNS باید دوباره آزمایش شود.
